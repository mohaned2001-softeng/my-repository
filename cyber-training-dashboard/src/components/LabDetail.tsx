...
const LabDetail: React.FC<LabDetailProps> = ({ labId, onBack, onLogin }) => {
  ...
  if (!labs.length) {
    return <div className="min-h-screen bg-[#0A0E27] pt-24 text-white text-center">Labs are loading. Please wait...</div>;
  }
          ...
          <div className="lg:col-span-2">
            <div className="bg-[#111827] rounded-2xl overflow-hidden border border-gray-800">
              <div className="relative h-64 md:h-80">
                <img src={getImageUrl(lab.image_url)} alt={lab.title} className="w-full h-full object-cover" />
              ...
              <div className="p-6">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      <div className="bg-[#0A0E27] p-4 rounded-lg text-center">
                        <div className="text-2xl font-bold text-red-500">{lab.estimated_time}</div>
                        <div className="text-gray-400 text-sm">Duration</div>
                      </div>
                      <div className="bg-[#0A0E27] p-4 rounded-lg text-center">
                        <div className="text-2xl font-bold text-green-500">{lab.skills?.length ?? 0}</div>
                ...
                {activeTab === 'writeup' && (
                  <div className="text-center py-8">
                    {user ? (
                      <a href={lab.writeup_url} target="_blank" rel="noopener noreferrer" className="inline-block px-8 py-4 bg-red-600 hover:bg-red-700 text-white font-semibold rounded-lg transition-colors">
          ...
          <div className="space-y-6">
            <div className="bg-[#111827] rounded-2xl p-6 border border-gray-800">
              ...
              <div className="space-y-3">
                {user ? (
                  <a href={lab.writeup_url} target="_blank" rel="noopener noreferrer" className="block w-full py-3 bg-red-600 hover:bg-red-700 text-white font-semibold rounded-lg text-center transition-colors">
                    View Writeup
                  </a>
                ) : (
            ...
            {relatedLabs.length > 0 && (
              <div className="bg-[#111827] rounded-2xl p-6 border border-gray-800">
                ...
                <div className="space-y-3">
                  {relatedLabs.map(rl => (
                    <div key={rl.id ?? rl.title} className="flex gap-3 p-2 bg-[#0A0E27] rounded-lg cursor-pointer hover:bg-gray-800" onClick={() => window.location.reload()}>
                      <img src={getImageUrl(rl.image_url)} alt={rl.title} className="w-12 h-12 rounded object-cover" />
                      <div>
